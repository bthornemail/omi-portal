...
## eMMC Image Size Formula

The eMMC/QEMU state-image carrier generated by `make emmc-state` uses the
polyharmonic governor as a deterministic storage layout:

```text
3 global headers × 512 bytes
+ 5 governor planes × (128-byte plane descriptor + 5040×8-byte payload)

= 1536 + 5 × (128 + 40320)
= 203776 bytes
```

The 128-byte plane descriptor is intentional. It completes each `5040×8`
governor payload to a 512-byte-aligned `40448` byte plane extent. The current
payload content is:

```text
32-byte .o word
+ 40288 bytes reserved zero payload space
= 40320 bytes
```

RPMB receipt proof covers the ordered plane descriptor hashes, the matching
`.o` payload word, and the reserved-zero payload invariant. Userdata carries
and RPMB accepts.
